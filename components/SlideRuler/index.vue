...
const handleDreawCanvas = () => {
  ...
  currentValue.value =
    currentValue.value > minValue
      ...
      : minValue;
  currentValue.value =
    (Math.round((currentValue.value * 10) / precision) * precision) / 10;
  emit("change", currentValue.value);
  let diffCurrentMin = ((currentValue.value - minValue) * divide) / precision,
    startValue =
      currentValue.value - Math.floor(canvasWidth / 2 / divide) * precision;
  startValue =
    startValue > minValue
      ? startValue < maxValue
        ? startValue
  ...
  let origin = {
    x:
      diffCurrentMin > canvasWidth / 2
        ? (canvasWidth / 2 -
            ((currentValue.value - startValue) * divide) / precision) *
          2
...
const handleTouchMove = e => {
  ...
  if (
    ...
    Math.abs(Math.round(deltaX / props.divide)) > 0
  ) {
